...
var handleLoadPage = function(hash) {
    ...
    window.track(targetUrl);
    targetUrl = targetUrl.replace(/&?contentOnly=([^&]$|[^&]*)/i, ""); //strip away contentonly from url
...
/* 21. auto filled all input under a table column
 ------------------------------------------------ */
var handleTableColumnInputFilling = function() {
    $('th > button.fillThisColumn').click(function(){
        var val = prompt('Enter value to be applied for all in this column.');

        if (val) {
            var $tr = $(this).parents("tr"), $th = $(this).closest("th"), $table = $(this).closest(".table");
            var colIndex = $tr.children("th").index($th);
            var tds = "td:nth-child(" + (colIndex + 1)+ ")";

            $table.find(tds).each(function(){
...
/* 22. auto filled all checkbox under a table column
 ------------------------------------------------ */
var handleTableColumnCheckboxFilling = function() {

    $('th > input[type=checkbox].fillThisColumn').click(function () {
...
var App = function () {
	...
	return {
		//main function
		init: function () {
            handleTableColumnInputFilling();
            handleTableColumnCheckboxFilling();
		...
		handleSlimScroll: handleSlimScroll,
		generateSlimScroll: generateSlimScroll,
